// AeneasSoft Email System — all emails via Resend
...
export async function sendWelcomeEmail(to: string, apiKey: string): Promise<void> {
  ...
  if (!resend) { console.log(`AeneasSoft Email (no Resend): Welcome → ${to}`); return; }
...
export async function sendResetPasswordEmail(to: string, resetToken: string): Promise<void> {
  ...
  if (!resend) { console.log(`AeneasSoft Email (no Resend): Reset → ${to} | ${resetUrl}`); return; }
...
export async function sendPaymentFailedEmail(to: string): Promise<void> {
  const resend = getResend();
  if (!resend) { console.log(`AeneasSoft Email (no Resend): PaymentFailed → ${to}`); return; }
...
export async function sendSubscriptionCancelledEmail(to: string): Promise<void> {
  const resend = getResend();
  if (!resend) { console.log(`AeneasSoft Email (no Resend): SubscriptionCancelled → ${to}`); return; }
...
export async function sendAlertEmail(to: string, alertName: string, value: number, threshold: number): Promise<void> {
  const resend = getResend();
  if (!resend) { console.log(`AeneasSoft Email (no Resend): Alert → ${to}: ${alertName}`); return; }
...
export async function sendContactEmail(name: string, email: string, company: string, message: string, type: string = 'contact'): Promise<void> {
  ...
  if (!resend) { console.log(`AeneasSoft Email (no Resend): ${subject} → ${SALES_EMAIL}`); return; }
...
const SUPPORT_EMAIL = process.env.SUPPORT_EMAIL || 'leonhard.hampe@aeneassoft.com';
...
export async function sendSupportEmail(data: {
  ...
  const resend = getResend();
  const subj = `[Support] ${data.subject} — von ${data.name}`;
  const body = `
    <h2>Neue Support-Anfrage</h2>
    <p><strong>Von:</strong> ${data.name} (${data.email})</p>
    <p><strong>User ID:</strong> ${data.userId || 'nicht eingeloggt'}</p>
    <hr/>
    <p>${data.message.replace(/\n/g, '<br/>')}</p>
    <hr/>
    <p style="color:#888;font-size:12px;">Gesendet: ${new Date().toISOString()}</p>
  `;

  if (!resend) { console.log(`AeneasSoft Email (no Resend): ${subj} → ${SUPPORT_EMAIL}`); return; }

  await resend.emails.send({ from: FROM, to: SUPPORT_EMAIL, replyTo: data.email, subject: subj, html: body });
}

export async function sendSupportConfirmation(to: string, name: string, message: string): Promise<void> {
  const resend = getResend();
  const subj = 'Wir haben deine Anfrage erhalten';

  if (!resend) { console.log(`AeneasSoft Email (no Resend): SupportConfirmation → ${to}`); return; }
  ...
  await resend.emails.send({
    ...
    html: `
      <h2>Hallo ${name},</h2>
      <p>vielen Dank für deine Nachricht. Wir haben deine Support-Anfrage erhalten und melden uns innerhalb von 24 Stunden.</p>
      <p><strong>Deine Nachricht:</strong></p>
      <blockquote style="border-left:3px solid #00B4FF;padding-left:12px;color:#666;">${message.replace(/\n/g, '<br/>')}</blockquote>
      <p>Das AeneasSoft Team<br/>support@aeneassoft.com</p>
    `,
  });
}
