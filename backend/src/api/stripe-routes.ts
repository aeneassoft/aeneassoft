// AeneasSoft Stripe Routes — Checkout, Webhooks, Billing Portal
...
export async function registerStripeRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/stripe/webhook', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!webhookSecret) {
      fastify.log.error('AeneasSoft STRIPE_WEBHOOK_SECRET not configured');
    ...
    try {
      ...
      fastify.log.error({ err }, 'AeneasSoft Stripe webhook signature verification failed');
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (email) {
          ...
          fastify.log.info(`AeneasSoft User ${email} upgraded to ${plan}`);
        }
        break;
      }
      ...
      case 'customer.subscription.deleted': {
        ...
        if (user) {
          ...
          sendSubscriptionCancelledEmail(user.email).catch(err =>
            fastify.log.error({ err }, 'AeneasSoft Failed to send subscription cancelled email'));
          fastify.log.info(`AeneasSoft Subscription deleted for ${user.email} — downgraded to free`);
        } else {
          fastify.log.warn(`AeneasSoft Subscription deleted for unknown customer ${customerId}`);
        }
        break;
      }
      ...
      case 'invoice.payment_failed': {
        ...
        fastify.log.warn(`AeneasSoft Payment failed for customer ${invoice.customer}`);
        const customerEmail = invoice.customer_email;
        if (customerEmail) {
          sendPaymentFailedEmail(customerEmail).catch(err =>
            fastify.log.error({ err }, 'AeneasSoft Failed to send payment-failed email'));
        }
        break;
      }

      default:
        fastify.log.info(`AeneasSoft Unhandled Stripe event: ${event.type}`);
    }
