...
export async function registerRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  // GET /api/traces/:trace_id/graph — React Flow nodes + edges
  fastify.get<{ Params: { trace_id: string } }>('/api/traces/:trace_id/graph', async (request, reply) => {
    try {
      ...
      // Reconstruct link data on spans from causal_links table
      ...
      const costByAgent = costBreakdownByAgent(spans);
    ...
    try {
      ...
      if (span.links) {
        ...
        for (const link of span.links) {
          ...
          await fetch(`${CLICKHOUSE_URL}/?database=${db}`, {
            method: 'POST',
            body: query,
          });
        }
  ...
  // GET /api/traces/:trace_id/compliance-report — PDF download (Task 10)
  fastify.get<{ Params: { trace_id: string } }>('/api/traces/:trace_id/compliance-report', async (request, reply) => {
    try {
      ...
      // Dynamic import to avoid breaking if pdfkit isn't available
