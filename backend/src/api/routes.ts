...
export async function registerRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get('/api/traces', async (request, reply) => {
    try {
      ...
      const { traces, total } = await queryTraces(CLICKHOUSE_URL, orgId, params.limit, params);
      ...
      return reply.send({ traces: result, total, limit: params.limit, offset: params.offset });
    } catch (err: any) {
      fastify.log.error({ err }, 'AeneasSoft Failed to query traces');
      return reply.status(500).send({ error: 'Failed to query traces' });
    }
  });

  // GET /api/traces/filters — distinct values for filter dropdowns
  fastify.get('/api/traces/filters', async (request, reply) => {
    try {
      const orgId = request.orgId || 'default';
      const { queryDistinctFilterValues } = await import('../db/clickhouse');
      const filters = await queryDistinctFilterValues(CLICKHOUSE_URL, orgId);
      return reply.send(filters);
    } catch (err: any) {
      fastify.log.error({ err }, 'AeneasSoft Failed to query filter values');
      return reply.status(500).send({ error: 'Failed to query filter values' });
    }
  });
  ...
  fastify.get<{ Params: { trace_id: string } }>('/api/traces/:trace_id/graph', async (request, reply) => {
    try {
      ...
      const orgId = request.orgId || 'default';
      const spans = await queryTraceSpans(CLICKHOUSE_URL, trace_id, orgId);
      ...
      fastify.log.error({ err }, 'AeneasSoft Failed to build graph');
  ...
  fastify.get<{ Params: { trace_id: string } }>('/api/traces/:trace_id/spans', async (request, reply) => {
    try {
      ...
      fastify.log.error({ err }, 'AeneasSoft Failed to query spans');
  ...
  fastify.get('/api/metrics', async (request, reply) => {
    try {
      const orgId = request.orgId || 'default';
      const m = await queryMetrics(CLICKHOUSE_URL, orgId);
      return reply.send({
      ...
      fastify.log.error({ err }, 'AeneasSoft Failed to query metrics');
  ...
  fastify.get('/api/cost/daily', async (request, reply) => {
    try {
      const q = request.query as Record<string, string>;
      const orgId = request.orgId || 'default';
      const data = await queryCostDaily(CLICKHOUSE_URL, orgId, q.from, q.to);
      return reply.send({ cost: data });
    } catch (err: any) {
      fastify.log.error({ err }, 'AeneasSoft Failed to query daily cost');
  ...
  fastify.get('/api/cost/by-agent', async (request, reply) => {
    try {
      const q = request.query as Record<string, string>;
      const orgId = request.orgId || 'default';
      const data = await queryCostByAgent(CLICKHOUSE_URL, orgId, q.from, q.to);
      return reply.send({ cost: data });
    } catch (err: any) {
      fastify.log.error({ err }, 'AeneasSoft Failed to query agent cost');
  ...
  fastify.get('/api/cost/by-model', async (request, reply) => {
    try {
      const q = request.query as Record<string, string>;
      const orgId = request.orgId || 'default';
      const data = await queryCostByModel(CLICKHOUSE_URL, orgId, q.from, q.to);
      return reply.send({ cost: data });
    } catch (err: any) {
      fastify.log.error({ err }, 'AeneasSoft Failed to query model cost');
  ...
  // POST /api/ingest — direct span ingestion (stricter rate limit: 200/min per IP)
  fastify.post('/api/ingest', { config: { rateLimit: { max: 200, timeWindow: '1 minute' } } }, async (request, reply) => {
    try {
      // ── Plan limit check ────────────────────────────────────────────────────
      ...
      const { insertSpanBatch } = await import('../db/clickhouse');
      await insertSpanBatch(CLICKHOUSE_URL, [span], orgId);
      ...
      fastify.log.error({ err }, 'AeneasSoft Failed to ingest span');
  ...
  fastify.get<{ Params: { trace_id: string } }>('/api/traces/:trace_id/compliance-score', async (request, reply) => {
    try {
      ...
      fastify.log.error({ err }, 'AeneasSoft Failed to calculate compliance score');
  ...
  fastify.get<{ Params: { trace_id: string } }>('/api/traces/:trace_id/compliance-report', async (request, reply) => {
    try {
      ...
      const user = request.userId ? await findUserById(CLICKHOUSE_URL, request.userId) : null;
      const isDemo = !user || user.plan === 'free';
      const pdfBuffer = await generateComplianceReport(trace_id, spans, { isDemo });
      reply.header('Content-Type', 'application/pdf');
      reply.header('Content-Disposition', `attachment; filename="compliance-report-${trace_id.substring(0, 8)}.pdf"`);
      return reply.send(pdfBuffer);
    } catch (err: any) {
      fastify.log.error({ err }, 'AeneasSoft Failed to generate compliance report');
  ...
  fastify.post('/api/keys', async (request, reply) => {
    try {
      ...
      fastify.log.error({ err }, 'AeneasSoft Failed to create API key');
  ...
  fastify.delete<{ Params: { id: string } }>('/api/keys/:id', async (request, reply) => {
    try {
      ...
      fastify.log.error({ err }, 'AeneasSoft Failed to delete API key');
  ...
  fastify.get('/api/keys', async (request, reply) => {
    try {
      ...
      fastify.log.error({ err }, 'AeneasSoft Failed to list API keys');
