// AeneasSoft Billing Route — GET /api/billing
  ...
  fastify.get('/api/billing', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (user.stripe_subscription_id) {
      ...
      if (stripe) {
        try {
          ...
          fastify.log.warn({ err }, 'AeneasSoft Failed to fetch Stripe subscription');
        }
      }
    }
    ...
    return reply.send({
      ...
      usage: {
        ...
        limit: user.plan in PLAN_LIMITS ? PLAN_LIMITS[user.plan] : PLAN_LIMITS.free,
