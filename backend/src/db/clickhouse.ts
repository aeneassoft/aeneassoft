...
export const CREATE_CB_STATE_TABLE = `
CREATE TABLE IF NOT EXISTS circuit_breaker_state (
  org_id         String,
  state          LowCardinality(String) DEFAULT 'closed',
  paused_until   Nullable(DateTime),
  budget_override Nullable(Float64),
...
// Helper: execute a ClickHouse query and throw on failure
async function chExec(url: string, db: string, body: string, label: string): Promise<void> {
  const res = await fetch(`${url}/?database=${db}`, { method: 'POST', body });
  if (!res.ok) {
    const text = await res.text().catch(() => 'unknown error');
    throw new Error(`ClickHouse ${label} failed (${res.status}): ${text}`);
  }
}
...
export async function initClickHouse(url: string): Promise<void> {
  ...
  // Create database
  const dbRes = await fetch(`${url}/?query=${encodeURIComponent(`CREATE DATABASE IF NOT EXISTS "${db}"`)}`);
  if (!dbRes.ok) {
    const text = await dbRes.text().catch(() => 'unknown');
    throw new Error(`ClickHouse CREATE DATABASE failed (${dbRes.status}): ${text}`);
  }

  // Create tables — fail loudly if any creation fails
  await chExec(url, db, CREATE_SPANS_TABLE, 'agent_spans');
  await chExec(url, db, CREATE_CAUSAL_LINKS_TABLE, 'causal_links');
  await chExec(url, db, CREATE_API_KEYS_TABLE, 'api_keys');
  await chExec(url, db, CREATE_USERS_TABLE, 'users');
  await chExec(url, db, CREATE_ALERT_RULES_TABLE, 'alert_rules');
  await chExec(url, db, CREATE_ALERT_HISTORY_TABLE, 'alert_history');
  await chExec(url, db, CREATE_PASSWORD_RESETS_TABLE, 'password_resets');
  await chExec(url, db, CREATE_DAILY_COST_TABLE, 'daily_cost_mv');
  await chExec(url, db, CREATE_CB_STATE_TABLE, 'circuit_breaker_state');

  // Non-breaking column additions (IF NOT EXISTS)
  await chExec(url, db, `ALTER TABLE users ADD COLUMN IF NOT EXISTS name Nullable(String)`, 'users.name');
  await chExec(url, db, `ALTER TABLE alert_history ADD COLUMN IF NOT EXISTS rule_name Nullable(String)`, 'alert_history.rule_name');

  console.log(`AeneasSoft ClickHouse tables initialized (TTL: ${retentionDays} days)`);
}
...
function serializeValue(v: any): string | number {
  ...
  if (Array.isArray(v)) return `[${v.map((s) => `'${esc(String(s))}'`).join(',')}]`;
...
const VALID_NAME_SEARCH = /^[\w\s.\-/]{1,200}$/;
const VALID_SORT_FIELDS = new Set(['start_time', 'latency_ms', 'accumulated_cost_usd', 'prompt_tokens', 'name', 'status_code']);

function sanitizeFilter(value: string, pattern: RegExp): string | null {
  return pattern.test(value) ? value : null;
}

export async function queryTraces(url: string, orgId: string, limit = 20, params: TraceQueryParams = {}): Promise<{ traces: any[]; total: number }> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const safeOrg = sanitizeFilter(orgId, VALID_IDENTIFIER) || 'default';
  const conditions = ['parent_span_id IS NULL', `org_id = '${safeOrg}'`];
  if (params.status) {
  ...
  if (params.name) {
    const safe = sanitizeFilter(params.name, VALID_NAME_SEARCH);
    if (safe) {
      conditions.push(`name ILIKE '%${esc(safe)}%'`);
    }
  }
  const effectiveLimit = Math.min(Math.max(params.limit || limit, 1), 500);
  const effectiveOffset = Math.max(params.offset || 0, 0);
  const sortField = params.sort && VALID_SORT_FIELDS.has(params.sort) ? params.sort : 'start_time';
...
export async function queryDistinctFilterValues(url: string, orgId: string): Promise<{
  ...
  const [agentRes, modelRes] = await Promise.all([
    fetch(`${url}/?database=${db}`, {
      method: 'POST',
      body: `SELECT DISTINCT agent_id, agent_name FROM agent_spans WHERE org_id = '${safeOrg}' AND agent_id != '' ORDER BY agent_name LIMIT 200 FORMAT JSON`,
    }),
    fetch(`${url}/?database=${db}`, {
      method: 'POST',
      body: `SELECT DISTINCT model_name FROM agent_spans WHERE org_id = '${safeOrg}' AND model_name IS NOT NULL AND model_name != '' ORDER BY model_name LIMIT 200 FORMAT JSON`,
    }),
  ]);
  ...
  return {
    agents: (agentData.data || []).map((r: any) => ({ agent_id: r.agent_id, agent_name: r.agent_name || r.agent_id })),
    models: (modelData.data || []).map((r: any) => r.model_name),
  };
}

export async function queryHourlyCBStatus(url: string, orgId: string): Promise<{
  hour_cost: number;
  error_rate: number;
  error_window_calls: number;
  ...
  const [costRes, errorRes, callRes, agentRes] = await Promise.all([
    fetch(`${url}/?database=${db}`, { method: 'POST',
      body: `SELECT sum(accumulated_cost_usd) as v FROM agent_spans WHERE org_id='${safeOrg}' AND start_time >= now() - INTERVAL 1 HOUR FORMAT JSON` }),
    fetch(`${url}/?database=${db}`, { method: 'POST',
      body: `SELECT countIf(status_code='ERROR') / greatest(count(), 1) as rate, count() as total FROM agent_spans WHERE org_id='${safeOrg}' AND start_time >= now() - INTERVAL 5 MINUTE FORMAT JSON` }),
    fetch(`${url}/?database=${db}`, { method: 'POST',
      body: `SELECT count() as v FROM agent_spans WHERE org_id='${safeOrg}' AND start_time >= now() - INTERVAL 1 MINUTE FORMAT JSON` }),
    fetch(`${url}/?database=${db}`, { method: 'POST',
      body: `SELECT agent_id, agent_name, sum(accumulated_cost_usd) as cost FROM agent_spans WHERE org_id='${safeOrg}' AND start_time >= now() - INTERVAL 1 HOUR GROUP BY agent_id, agent_name ORDER BY cost DESC LIMIT 10 FORMAT JSON` }),
  ]);
  ...
  return {
    hour_cost: Number(costData.data?.[0]?.v) || 0,
    error_rate: Number(errorData.data?.[0]?.rate) || 0,
    error_window_calls: Number(errorData.data?.[0]?.total) || 0,
    calls_per_minute: Number(callData.data?.[0]?.v) || 0,
    by_agent: (agentData.data || []).map((r: any) => ({
      agent_id: r.agent_id,
...
// ── Circuit Breaker State (dashboard controls) ──────────────────────────────

export async function getCBState(url: string, orgId: string): Promise<{
  state: string; paused_until: string | null; budget_override: number | null; reason: string;
}> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const safeOrg = sanitizeFilter(orgId, VALID_IDENTIFIER) || 'default';
  const query = `SELECT * FROM circuit_breaker_state FINAL WHERE org_id = '${safeOrg}' LIMIT 1 FORMAT JSON`;
  ...
  return {
    ...
    budget_override: row?.budget_override ? Number(row.budget_override) : null,
    reason: row?.reason || '',
  };
}

export async function setCBState(
  url: string, orgId: string, state: string,
  pausedUntil?: string | null, budgetOverride?: number | null,
  ...
  const safeBy = esc(changedBy || 'dashboard');
  const pausedVal = pausedUntil ? `'${esc(pausedUntil)}'` : 'NULL';
  const budgetVal = budgetOverride != null ? String(budgetOverride) : 'NULL';
  const query = `INSERT INTO circuit_breaker_state (org_id, state, paused_until, budget_override, reason, changed_by) VALUES ('${safeOrg}', '${safeState}', ${pausedVal}, ${budgetVal}, '${safeReason}', '${safeBy}')`;
...
export async function queryTraceSpans(url: string, traceId: string, orgId?: string): Promise<any[]> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const safeId = assertHexId(traceId, 32);   // throws if not hex32
  const orgFilter = orgId ? ` AND org_id = '${sanitizeFilter(orgId, VALID_IDENTIFIER) || 'default'}'` : '';
  const query = `
    SELECT *
    FROM agent_spans
    WHERE trace_id = '${safeId}'${orgFilter}
...
export async function queryMetrics(url: string, orgId?: string): Promise<any> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const orgFilter = orgId ? `AND org_id = '${sanitizeFilter(orgId, VALID_IDENTIFIER) || 'default'}'` : '';
  ...
  const [mainRes, monthRes] = await Promise.all([
    fetch(`${url}/?database=${db}`, {
      ...
      body: `
        ...
        FROM agent_spans
        WHERE start_time >= now() - INTERVAL 24 HOUR ${orgFilter}
        FORMAT JSON
      `,
    }),
    fetch(`${url}/?database=${db}`, {
      method: 'POST',
      body: `SELECT countDistinct(trace_id) as traces_this_month FROM agent_spans WHERE toYYYYMM(start_time) = toYYYYMM(now()) ${orgFilter} FORMAT JSON`,
    }),
  ]);
...
export async function createApiKey(url: string, rawKey: string, orgId: string, label: string = ''): Promise<void> {
  ...
  const query = `INSERT INTO api_keys (key_hash, org_id, label) VALUES ('${keyHash}', '${esc(orgId)}', '${esc(label)}')`;
  await fetch(`${url}/?database=${db}`, { method: 'POST', body: query });
}
...
export async function queryCostDaily(url: string, orgId?: string, from?: string, to?: string): Promise<any[]> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const conditions: string[] = [];
  if (orgId) { const safe = sanitizeFilter(orgId, VALID_IDENTIFIER); if (safe) conditions.push(`org_id = '${safe}'`); }
...
export async function queryCostByAgent(url: string, orgId?: string, from?: string, to?: string): Promise<any[]> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const conditions: string[] = [];
  if (orgId) { const safe = sanitizeFilter(orgId, VALID_IDENTIFIER); if (safe) conditions.push(`org_id = '${safe}'`); }
  if (from) { const safe = sanitizeFilter(from, VALID_DATE); if (safe) conditions.push(`day >= '${safe}'`); }
...
export async function queryCostByModel(url: string, orgId?: string, from?: string, to?: string): Promise<any[]> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const conditions: string[] = [];
  if (orgId) { const safe = sanitizeFilter(orgId, VALID_IDENTIFIER); if (safe) conditions.push(`org_id = '${safe}'`); }
  if (from) { const safe = sanitizeFilter(from, VALID_DATE); if (safe) conditions.push(`day >= '${safe}'`); }
...
export async function createUser(
  ...
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const safeName = name ? `'${esc(name)}'` : 'NULL';
  const query = `INSERT INTO users (id, email, password_hash, org_id, name) VALUES ('${esc(id)}', '${esc(email)}', '${esc(passwordHash)}', '${esc(orgId)}', ${safeName})`;
  await fetch(`${url}/?database=${db}`, { method: 'POST', body: query });
}
...
export async function findUserByEmail(url: string, email: string): Promise<UserRow | null> {
  ...
  const query = `SELECT * FROM users FINAL WHERE email = '${esc(email)}' LIMIT 1 FORMAT JSON`;
  const res = await fetch(`${url}/?database=${db}`, { method: 'POST', body: query });
  const data = (await res.json()) as any;
...
export async function findUserByStripeCustomerId(url: string, customerId: string): Promise<UserRow | null> {
  ...
  const query = `SELECT * FROM users FINAL WHERE stripe_customer_id = '${esc(customerId)}' LIMIT 1 FORMAT JSON`;
  const res = await fetch(`${url}/?database=${db}`, { method: 'POST', body: query });
  const data = (await res.json()) as any;
  return data.data?.[0] || null;
}
...
export async function updateUser(
  ...
  const query = `INSERT INTO users (id, email, password_hash, org_id, plan, stripe_customer_id, stripe_subscription_id, updated_at) VALUES ('${esc(updated.id)}', '${esc(updated.email)}', '${esc(updated.password_hash)}', '${esc(updated.org_id)}', '${esc(updated.plan)}', ${updated.stripe_customer_id ? `'${esc(updated.stripe_customer_id)}'` : 'NULL'}, ${updated.stripe_subscription_id ? `'${esc(updated.stripe_subscription_id)}'` : 'NULL'}, '${ts}')`;
  await fetch(`${url}/?database=${db}`, { method: 'POST', body: query });
}
...
export async function createPasswordReset(url: string, tokenHash: string, email: string, expiresAt: string): Promise<void> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const query = `INSERT INTO password_resets (token, email, expires_at) VALUES ('${tokenHash}', '${esc(email)}', '${esc(expiresAt)}')`;
  await fetch(`${url}/?database=${db}`, { method: 'POST', body: query });
}
...
export async function createAlertRule(url: string, rule: Omit<AlertRule, 'created_at'>): Promise<void> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const query = `INSERT INTO alert_rules (id, org_id, name, trigger_type, threshold, time_window_minutes, action_type, action_target, is_active) VALUES ('${esc(rule.id)}', '${esc(rule.org_id)}', '${esc(rule.name)}', '${esc(rule.trigger_type)}', ${Number(rule.threshold)}, ${Number(rule.time_window_minutes)}, '${esc(rule.action_type)}', '${esc(rule.action_target)}', ${rule.is_active ? 1 : 0})`;
  await fetch(`${url}/?database=${db}`, { method: 'POST', body: query });
}

export async function deleteAlertRule(url: string, ruleId: string, orgId: string): Promise<void> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  if (!VALID_UUID.test(ruleId)) return;
  const safeOrg = sanitizeFilter(orgId, VALID_IDENTIFIER) || 'default';
  const query = `ALTER TABLE alert_rules DELETE WHERE id = '${ruleId}' AND org_id = '${safeOrg}'`;
  await fetch(`${url}/?database=${db}`, { method: 'POST', body: query });
}
...
export async function saveAlertHistory(url: string, entry: { id: string; rule_id: string; rule_name?: string; org_id: string; value: number; message: string }): Promise<void> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  const safeRuleName = entry.rule_name ? `'${esc(entry.rule_name)}'` : 'NULL';
  const query = `INSERT INTO alert_history (id, rule_id, rule_name, org_id, value, message) VALUES ('${esc(entry.id)}', '${esc(entry.rule_id)}', ${safeRuleName}, '${esc(entry.org_id)}', ${Number(entry.value)}, '${esc(entry.message)}')`;
  await fetch(`${url}/?database=${db}`, { method: 'POST', body: query });
}
...
export async function updateAlertIsActive(url: string, id: string, orgId: string, isActive: 0 | 1): Promise<void> {
  const db = process.env.CLICKHOUSE_DB || 'aeneassoft';
  if (!VALID_UUID.test(id)) return;
  const safeOrg = sanitizeFilter(orgId, VALID_IDENTIFIER) || 'default';
  const query = `ALTER TABLE alert_rules UPDATE is_active = ${isActive} WHERE id = '${id}' AND org_id = '${safeOrg}'`;
  await fetch(`${url}/?database=${db}`, { method: 'POST', body: query });
}
