// AeneasSoft Alert Worker — checks alert conditions every 60 seconds
...
async function triggerAlert(rule: any, value: number): Promise<void> {
  ...
  if (rule.action_type === 'webhook') {
    try {
      ...
      console.error(`AeneasSoft Failed to send webhook alert to ${rule.action_target}:`, err);
    }
  }

  if (rule.action_type === 'email') {
    sendAlertEmail(rule.action_target, rule.name, value, rule.threshold).catch(err =>
      console.error(`AeneasSoft Failed to send alert email to ${rule.action_target}:`, err));
  }

  console.log(`AeneasSoft Alert triggered: ${message}`);
}
...
async function checkAlerts(): Promise<void> {
  try {
    ...
    console.error('AeneasSoft Alert worker error:', err);
  }
}

export function startAlertWorker(): void {
  if (intervalId) return;
  console.log('AeneasSoft Alert worker started (60s interval)');
...
export function stopAlertWorker(): void {
  if (intervalId) {
    ...
    intervalId = null;
    console.log('AeneasSoft Alert worker stopped');
  }
}
