// AeneasSoft Auth Middleware — Dual authentication: JWT + API Key
...
export async function registerAuthMiddleware(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.addHook('onRequest', async (request: FastifyRequest, reply: FastifyReply) => {
    // LOCAL_MODE: skip all auth, set default org/user
    // Only activates when explicitly set to 'true' — never auto-enabled
    const localMode = process.env.LOCAL_MODE === 'true';
    if (localMode) {
      request.orgId = 'local';
      request.userId = 'local-user';
      return;
    }
