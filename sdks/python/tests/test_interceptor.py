"""
AeneasSoft Python SDK — Tests for universal HTTP interceptor
"""
import json
import time
import pytest
...
# ── Active Defense integration with interceptor ──────────────────────────────

class TestInterceptorCircuitBreaker:
    def test_check_monitors_called_for_ai_url(self):
        """_check_monitors is called before AI provider calls."""
        from agentwatch.interceptor import _check_monitors, _record_monitors

        with patch("agentwatch.interceptor._check_monitors") as mock_check, \
             patch("agentwatch.interceptor._record_monitors"):
            # Import _sync_send indirectly by patching httpx
            from agentwatch.interceptor import _patch_httpx, _detect_provider
            info = _detect_provider("https://api.openai.com/v1/chat/completions")
            assert info is not None
            # _check_monitors should be importable and callable
            mock_check.assert_not_called()  # Not called until actual send

    def test_check_monitors_blocks_before_http(self):
        """CircuitBreakerException prevents HTTP call from being made."""
        from agentwatch.circuit_breaker import get_global_monitor, CircuitBreakerException

        gm = get_global_monitor()
        gm.configure(budget=1.0, block_on_threshold=True)
        # Exceed budget
        for _ in range(3):
            gm.record_result(cost_usd=0.50, is_error=False)

        # _check_monitors should raise
        from agentwatch.interceptor import _check_monitors
        with pytest.raises(CircuitBreakerException):
            _check_monitors("https://api.openai.com/v1/chat/completions")

        # Reset
        gm.configure()

    def test_record_monitors_tracks_errors(self):
        """_record_monitors records errors on the global monitor."""
        from agentwatch.circuit_breaker import get_global_monitor
        from agentwatch.interceptor import _record_monitors

        gm = get_global_monitor()
        gm.configure(max_error_rate=0.9)  # High threshold so it doesn't block
        gm._error_window.clear()
        gm._cost_window.clear()

        _record_monitors(0.01, is_error=True)
        _record_monitors(0.01, is_error=False)

        assert len(gm._error_window) == 2
        assert gm._error_window[0][1] is True
        assert gm._error_window[1][1] is False
