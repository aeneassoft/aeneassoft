// [PRODUCTNAME] Node.js Universal HTTP Interceptor
...
// Hosts handled by SDK-level patchers — skip to avoid duplicates
  ...
  setImmediate(() => {
    try {
      ...
      const span: Record<string, unknown> = {
        ...
        agent_name: '[PRODUCTNAME] Universal Interceptor',
      ...
      if (!_zdr) {
        ...
        if (output) span.output = output.slice(0, 2000);
      }
...
function patchModule(mod: typeof http | typeof https): void {
  ...
  function patchedRequest(
    ...
    // Pass through if not an AI provider or SDK already handles it
    if (!providerInfo || isSdkHandled(hostname)) {
    ...
    // Capture outgoing body
    const origWrite = req.write.bind(req);
    const origEnd = req.end.bind(req);

    req.write = function (chunk: unknown, ...args: unknown[]) {
      if (chunk) {
        reqChunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk as string));
      }
    ...
    req.on('response', (res: http.IncomingMessage) => {
      ...
      res.on('data', (chunk: Buffer) => respChunks.push(chunk));
      res.on('end', () => {
